...
bool Bass::source(const string& filename) {
  ...
  lstring commenttext = data.split("\n");
  string thething = "";
  for(unsigned lineNumber = 0; lineNumber < lines.size(); lineNumber++) {
	thething.append(lines[lineNumber]);
  }
  
  for(unsigned lineNumber = 0; lineNumber < lines.size(); lineNumber++) {
	    nall::maybe<unsigned int> globPosition;
    if(auto position = lines[lineNumber].find("//")) lines[lineNumber].resize(position());  //remove line comments (wtf byuu, why no block comments?)
